...
class Helper {
	...
	public static function init_metrics($xhprof_data, $rep_symbol, $sort, $diff_report = false) {
		Html::$diff_mode = $diff_report;
		if (!empty($sort)) {
			if (array_key_exists($sort, Html::$sortable_columns)) {
				Html::$sort_col = $sort;
			} else {
		...
		if (!isset($xhprof_data["main()"]["wt"])) {
			...
			Html::$display_calls = false;
		} else {
			Html::$display_calls = true;
		}

		// parent/child report doesn't support exclusive times yet.
		// So, change sort hyperlinks to closest fit.
		if (!empty($rep_symbol)) {
			Html::$sort_col = str_replace("excl_", "", Html::$sort_col);
		}

		if (Html::$display_calls) {
			Html::$stats = array("fn", "ct", "Calls%");
		} else {
			Html::$stats = array("fn");
		}

		Html::$pc_stats = Html::$stats;
	...
	public static function xhprof_param_init($params) {
		...
		foreach ($params as $k => $v) {
			switch ($v[0]) {
				case self::XHPROF_STRING_PARAM:
					$p = self::xhprof_get_string_param($k, $v[1]);
					break;
				case self::XHPROF_UINT_PARAM:
					$p = self::xhprof_get_uint_param($k, $v[1]);
					break;
				case self::XHPROF_FLOAT_PARAM:
					$p = self::xhprof_get_float_param($k, $v[1]);
					break;
				case self::XHPROF_BOOL_PARAM:
