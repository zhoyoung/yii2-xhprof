...
// Supported ouput format
	...
	/**
	 * Send an HTTP header with the response. You MUST use this public static function instead
	...
	public static function xhprof_http_header($name, $value) {
		...
		if (!$name) {
			Helper::xhprof_error('http_header usage');
			return null;
		}

		if (!is_string($value)) {
			Helper::xhprof_error('http_header value not a string');
		}
	...
	/**
	 * Generate image according to DOT script. This public static function will spawn a process
	...
	public static function xhprof_generate_dot_script($raw_data, $threshold, $source, $page,
		...
		$sym_table = Helper::xhprof_compute_flat_info($raw_data, $totals);
			...
			while ($node) {
				...
				if (isset($children_table[$node])) {
					...
					foreach ($children_table[$node] as $child) {
						...
						if ($max_child === null ||
							abs($raw_data[Helper::xhprof_build_parent_child_key($node,
								$child)]["wt"]) >
							abs($raw_data[Helper::xhprof_build_parent_child_key($node,
								$max_child)]["wt"])
						) {
					...
					if ($max_child !== null) {
						$path[$max_child] = true;
						$path_edges[Helper::xhprof_build_parent_child_key($node, $max_child)] = true;
					}
					$node = $max_child;
				} else {
					$node = null;
		...
		// use the public static function to filter out irrelevant functions.
		if (!empty($func)) {
		...
		// also assign a unique integer id for each public static function to be generated. In the
		// meantime, find the public static function with the most exclusive time (potentially the
		...
		foreach ($raw_data as $parent_child => $info) {
			...
			if (isset($sym_table[$parent]) && isset($sym_table[$child]) &&
				...
				if ($critical_path &&
					isset($path_edges[Helper::xhprof_build_parent_child_key($parent, $child)])
				) {
					$linewidth = 10;
					$arrow_size = 2;
				}
	...
	public static function  xhprof_render_diff_image($xhprof_runs_impl, $run1, $run2,
		...
		$total1;
		$total2;
		/** @var XHProfRuns_Default $xhprof_runs_impl */
		...
		$symbol_tab1 = Helper::xhprof_compute_flat_info($raw_data1, $total1);
		$symbol_tab2 = Helper::xhprof_compute_flat_info($raw_data2, $total2);
	...
	public static function xhprof_get_content_by_run($xhprof_runs_impl, $run_id, $type,
		...
		if (!$raw_data) {
			Helper::xhprof_error("Raw data is empty");
			return "";
		}
